...
export class ARControls extends Behaviour {
  ...
  *showThenHideTouchCube() {
    ...
    while (true) {
      ...
      this.timer = Math.max(
        0,
        Math.min(this.timer - this.context.time.deltaTime * 1)
      );
      yield;
      if (this.timer <= 0.01) {
  ...
  *WaitThenReset() {
  ...
  start() {
    ...
    if (this.heightAlignScript == null) {
      this.heightAlignScript =
        this.gameObject.getComponent(AlignHeightToGround);
    }
    ...
    window.addEventListener("touchstart", (e) => {
      ...
      if (e.targetTouches.length == 1) {
        ...
        this.initialRotPos =
          this.activeTouchEvent.touches[0].clientX -
          this.activeTouchEvent.touches[1].clientX;
        var b =
          this.activeTouchEvent.touches[0].clientY -
          this.activeTouchEvent.touches[1].clientY;
        this.initialDist = Mathf.clamp(
          Math.sqrt(this.initialRotPos * this.initialRotPos + b * b) / 350,
          0,
          10
        );
    ...
    mat.color = new THREE.Color(0.4, 0.4, 0.4);
    line.layers.set(2);
    line.name = "line";
    line.scale.y = 1;
    this._groundLine = line;

    const geometry = new THREE.SphereGeometry(0.5, 22, 22);
    ...
    if (this.isTouch == true) {
      ...
      for (const hit of rc) {
        if (hit.distance > 0 && GameObject.isActiveInHierarchy(hit.object)) {
          ...
          this.arctrl = GameObject.getComponentInParent(
            this.mainTarget,
            ARControls
          );
          if (this.arctrl != null) {
  ...
  update() {
    if (this.arOnly == true && this.context.isInAR == true) {
